    ...
    def encode_gc_content_after_c1(self):
        ...
        for strand in data:
            ...
            if type(x) is tuple:
                ...
                if strand[0:13] in dict_mapping.keys():
                    dict_mapping[strand[0:13]][0] = dict_mapping[strand[0:13]][0] + 1
                else:
                    dict_mapping[strand[0:13]] = [1, 0]
            else:
                encoded_strand += x
            ...
            for i in range(13, self.information_bits_in_short_row, 13):
                ...
                if type(x) is tuple:
                    ...
                    if abs(((x0_count + gc_counter) / (encoded_strand_len + 7)) - 0.5) < abs(
                        ...
                        encoded_strand += x[0]
                        gc_counter += x0_count
                        if strand[i:i+13] in dict_mapping.keys():
                            dict_mapping[strand[i:i+13]][0] = dict_mapping[strand[i:i+13]][0] + 1
                        else:
                            dict_mapping[strand[i:i+13]] = [1, 0]
                    else:
                        encoded_strand += x[1]
                        gc_counter += x1_count
                        if strand[i:i+13] in dict_mapping:
                            dict_mapping[strand[i:i + 13]][1] = dict_mapping[strand[i:i+13]][1] + 1
                            if dict_mapping[strand[i:i+13]][1] >0 and dict_mapping[strand[i:i+13]][0] > 0:
                                print(strand[i:i+13])
                                print( dict_mapping[strand[i:i + 13]])
                        else:
                            dict_mapping[strand[i:i + 13]] = [0, 1]
                else:
                    encoded_strand += x
                    gc_counter += (x.count('G') + x.count('C'))
                    if strand[i:i+13] in dict_mapping.keys():
                    ...
                    for i in range(self.information_bits_in_short_row, self.information_bits_in_long_row, 15):
                        ...
                        if type(x) is tuple:
                            ...
                            if abs(((x0_count + gc_counter) / (encoded_strand_len + 8)) - 0.5) < abs(
                                ...
                                encoded_strand += x[0]
                                gc_counter += x0_count
                                if strand[i:i + 15] in dict_mapping.keys():
                                    dict_mapping[strand[i:i + 15]][0] = dict_mapping[strand[i:i+15]][0] + 1
                                else:
                                    dict_mapping[strand[i:i + 15]] = [1, 0]
                            else:
                                encoded_strand += x[1]
                                gc_counter += x1_count
                                if strand[i:i + 15] in dict_mapping.keys():
                                    dict_mapping[strand[i:i + 15]][1] = dict_mapping[strand[i:i+15]][1] + 1
                                    if dict_mapping[strand[i:i + 15]][1] > 0 and dict_mapping[strand[i:i + 15]][0] > 0:
                                        print(strand[i:i + 15])
                                        print(dict_mapping[strand[i:i + 15]])
                                else:
                                    dict_mapping[strand[i:i + 15]] = [0, 1]
                        else:
                            encoded_strand += x
                            gc_counter += (x.count('G') + x.count('C'))
                            if strand[i:i + 15] in dict_mapping.keys():
...
def encode(erasure_fraction=0.04, substitution_fraction=0.0075, almost_correct_fraction=0.016,
    ...
    # TODO: add indexer to the pipeline :) right now the set of indices is pre-made
    # The set of indices
    end = time.time()
    print(end-st)
    if is_indexer:
...
#encode(file_to_encode="/Users/omersabary/Desktop/DaniellaDvir/test_finale.nosync/two_files.nosync/rand_file.bin")
...
#generate_indices(encoded_file="/Users/omersabary/Desktop/DaniellaDvir/test_finale.nosync/Submission-Martine/PilotPool/pilot_strands.txt")